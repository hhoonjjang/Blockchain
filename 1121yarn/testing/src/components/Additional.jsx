...
export default function Additional() {
  ...
  useEffect(() => {
    ...
    console.log(ref.current.className); // Element에 접근 시 current 프로퍼티로 가져온다.
  }, []);

  const [testValue, setValue] = useState(0);
  //기존에 useEffect를 사용할 경우 어떤 상태(a)의 값이 변화 시 a를 기준으로
  // 다른 상태(b)의 값을 계산하여 정의하는 경우가 있다.
  // {ex axios를 통한 데이터 받기)
  ...
  // useMemo와 유사하나 그대로 testFunc에 정의한다.
  // 즉 testFunc는 함수이므로 호출할 때는 testFunc()로 사용해야한다.
  // useMemo와 마찬가지로 testValue의 값이 변경되었을 때 만 재정의한다.

  function testFunc1() {
    console.log(testValue + 100);
  }
  //위와 같이 function 등등으로 함수를 선언했을 때는 리랜더링 될 떄마다 함수를 다시
  // 정의한다. useCallback 을 사용시 두번쨰 매개변수의 상태값이 변화하지않으면
  // 재정의하지 않고 있던것을 사용한다.
  ...
  return (
    ...
    >
      {tValue}
      {/* useMemo로 정의한 변수 사용시 일반변수처럼가져온다. */}
    </div>
  );
}
