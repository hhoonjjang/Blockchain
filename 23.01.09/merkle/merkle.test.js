...
describe("이건 테스트의 그룹이다.", () => {
  ...
  // expect (확인할 내용) : 적혀있는 그대로 확인할 함수, 객체, 변수 등등을 매개변수로 전달한다.
  // 이것만 있으면 아무의미가 없다.
  ...
  it("이건 안된다., ()=>", () => {
    //   expect({ a: 1 }).toBe({ a: 1 });
    // 객체는 서로 다른것으로 취급한다. 같은 객체가 되려면 변수에 정의해야한다.
    const obj = { a: 1 };
    expect(obj).toBe(obj);
  });
  ...
  describe("toEqual VS toStrictEqual", () => {
    // describe는 describe안에서도 사용할 수 있으며 하위 목록이 된다.
    // Strict : 엄격한
    it("하니씩 보자", () => {
      //   expect({ a: 1, b: undefined }).toBe({ a: 1, b: undefined });
      expect({ a: 1, b: undefined }).toEqual({ a: 1 });
      //   expect({ a: 1, b: undefined }).toStrictEqual({ a: 1 });
      // toStrictEqual은 내용자체가 모두 일치해야한다. 같아야한다.
    });
  });
});

describe("merkle 비교", () => {
  //   it("하나의 데이터 머클 암호화 확인", () => {
