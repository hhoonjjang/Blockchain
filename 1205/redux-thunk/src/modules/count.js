...
// 2. action이 함수인지 확인하여 함수라면 reducer에 전달하는 것이 아닌 action 함수 자체를 호출한다. >>확인은 thunk가 해준다. 전달하는 액션 함수는 async,await가 가능하다.
...
const asyncIncrement = () => {
  ...
  return async (dispatch, getState) => {
    ...
    console.log("3. asyncIncrement return fucniton.");
    try {
      // await를 사용한 함수의 호출이 에러가 발생할 수 도 있기 때문에 (reject) try, catch 구문을 사용한다.
...
export const reducer = (state = initialize, action) => {
  ...
  // 9. type에 따라서 처리 코드를 분류하여 처리한다.
  console.log("9.type = ", type);
  switch (type) {
    // 9-1. 현재는 increment를 호출중이다.
    case TYPE.INCREMENT:
      //10. return 된 state + 1 을 state(현재는 count)에 적용한다.
