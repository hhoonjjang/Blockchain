...
//  userAgent를 정규표현식을 사용해 원하는 정보만 가져올 수 있다.
// (다만, 라이브러리를 사용해서 쉽게 처리가 가능하다.)
// p/s 애플 계열은 IOS, iPhone, iPad, Mac 등으로만 나타난다.
// 유저 정보를 알고 싶을때 정보 수집한다.
...
// 데이터적 구조
// BOM / DOM feat.MVC (Model View Contoller)

// Node, Tag, Element란 무엇인가?
// Tag 는 HTML에서 사용하는 명령어의 이름을 뜻했다. ex) html, body, div 등등
//  Tag는 여는 Tag와 닫는 Tag로 사용한다. <div></div>
// Element는 뭐냐? >> 여는 Tag와 닫는 Tag를 모두 포함하는 내용이다.<<라고 했다.

// DOM(document) 내에서 정의되는 Tag의 내용이다.
// ex) document.getElementById('name') >> name 을 id로 갖는 Tag를 찾는다.
// 정확히는 Element를 찾는 것이다.

// Tag 와 Element의 차이는 무엇이냐? Tag는 이름 그 자체다. Element는 객체다.
// 즉 Javascript에서 HTML 구조를 수정하거나 내용을 추가하거나 등등에서 사용하는
// Tag에 대한 객체다.
...
// 객체 안에 있는 함수는 메소드(method)라고 부른다.
...
// outerHTML은 여는 태그와 다는 태그 사이의 데이터를 문자열로 받는다.
// 클래스는 getElementsByClassName
...
// const person = { name: "", age: 0, gneder: 0 };
