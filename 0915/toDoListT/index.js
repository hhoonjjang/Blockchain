...
// const로 list를 초기화한 이유: 메서드(함수)를 호출 할때마다  Javascript는 해당 코드를 실행하기 때문에 실행을 최소화하기 위해서 변수에 반환값을(return)을 정의하여 이후 다시 찾지 않고 바로 쓸 수 있도록 한다.

function deleteItem(num) {
  // 삭제를 위한 함수
  //   list.children[num].classList.add("delete");
  //   list의 자식들(Element) 중에 num번째 자식에 'delete'라는 클래스를 추가한다.
  // delete 클래스를 가진 엘레먼트는 CSS 스타일로 인해 가운데 줄이 생긴다.
  //   let startIdx = 0;
  //   let endIdx = 0;
  ...
  //   list.innerHTML = tempHTML.slice(0, startIdx) + tempHTML.slice(0, endIdx + 4);
...
document.getElementById("todo-add").onclick = () => {
  ...
  //입력된 값을 확인하기 위해 엘레먼트를 찾아 변수에 초기화한다.
  ...
  //  list, 할일 목록에 li엘레먼트를 추가한다.
  // 엘레먼트의 자식으로 버튼을 추가하여 list의 자식들의 길이(현재는 추가 되지 않았기 때문에 index처럼 사용가능)
  // 를 매개변수로 전달한다.
  input.value = "";
  //   입력된 값을 없앤다.
};
